...
/* MAP.CPP::setPartsPrio() calls insertCharPartsPrio(existingChar, part).
   That helper links the new part BEFORE the existing character.  The web
   list is kept in the same chain order and reversed only at paint time, so
   a tree/roof that covers an actor must be inserted before that character. */
if (!/mapPartBeforeActor\(part,candidate\)\)\{items\.splice\(index,0,entry\);inserted=true;break;\}/.test(worldActorPaintSource) ||
