...
(async()=>{
 for(const failure of ['none','throw','reject']){
  ...
  const state={phase:'world',polling:true,rideRequest:{timer:7},petActionRequest:{timer:8},transport:{close(options){closes++;assert.equal(options.keepalive,true);if(failure==='throw')throw Error('closed');if(failure==='reject')return Promise.reject(Error('closed'));return Promise.resolve();},logoutAndClose(){assert.fail('must not send record-point logout');}}};
  const cleared=[];const env={app:state,cancelConnectionRetry(){cancelled++;},window:{clearTimeout:id=>cleared.push(id),addEventListener:(name,fn)=>events[name]=fn}};
  vm.createContext(env);vm.runInContext(section('  function clearPetActionRequest(', '  function clearRideRequest('),env);vm.runInContext(section('  function clearRideRequest(', '  function receiveRideStatus('),env);vm.runInContext(section('  let pageUnloadLogoutSent=false;','  function returnToLogin('),env);
  ...
  assert.equal(closes,1);assert.equal(state.rideRequest,null);assert.equal(state.petActionRequest,null);assert.deepEqual(cleared,[7,8]);assert.equal(state.transport,null);assert.equal(state.polling,false);assert.equal(cancelled,2);
 }
