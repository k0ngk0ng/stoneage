...
8.5 `chat.cpp` 还包含两个条件编译快捷键：`Ctrl+V`（剪贴板粘贴）现在已在网页
恢复，失焦时也会把文本插入聊天缓冲区；`Ctrl+R` 只在 `_TELLCHANNEL` 下切换密语
频道并填入 `secretName`。2.5 客户端/服务端没有 `_TELLCHANNEL` 或对应封包，故
`Ctrl+R` 是 **1 项明确的服务端能力差异**，不会伪造到网页协议中。它不影响上述
39 项正式 2.5 本地功能统计。
