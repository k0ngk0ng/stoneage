...
static struct pl *QuizPlayer = NULL;
static int QuizPlayerCount = 0;
...
BOOL NPC_GetQuestion(int meindex, int *tbl, int capacity);
...
BOOL NPC_QuizInit( int meindex )
{
	...
	if( !NPC_GetQuestion( meindex, tbl, arraysizeof(tbl)) ) return FALSE;

	if( CHAR_getWorkInt( meindex, CHAR_WORK_QUIZNUM) > OLDNO ||
		CHAR_getWorkInt( meindex, CHAR_WORK_QUIZNUM) > ( tbl[0] - 1)){
		print(
	...
	switch( num) {
	  ...
	  case 2:
  		/*--丢奶件--*/
  		{
			...
			pl_ptr = NPC_QuizGetPlayer(meindex, p_no);
			if( pl_ptr == NULL || pl_ptr->talkerindex != talker ) return;
			memcpy(&PLAYER, pl_ptr, sizeof(PLAYER));
			...
			if( !NPC_GetQuestion( meindex, tbl, arraysizeof(tbl)) ) {
			...
			if( Quiz[questionno].answertype == 4) {
				...
				/*--蜇箕及橇须毛丢乒伉卞筏盛今六化云仁--*/
				memcpy(pl_ptr,&PLAYER,sizeof(PLAYER));
	...
	switch( seqno){
	  ...
	  case CHAR_WINDOWTYPE_QUIZ_MAIN:
	  {
		...
		struct pl PLAYER;
			...
			pl_ptr = NPC_QuizGetPlayer(meindex, p_no);
			if( pl_ptr == NULL || pl_ptr->talkerindex != talkerindex ) return;
			memcpy(&PLAYER,pl_ptr,sizeof(PLAYER));

			p_no = CHAR_getWorkInt( talkerindex, CHAR_WORKSHOPRELEVANT);
			old_no = CHAR_getWorkInt( talkerindex, CHAR_WORKSHOPRELEVANTSEC)-1;
			if( old_no < 0 || old_no >= OLDNO ) return;
...
BOOL QUIZ_initQuiz( char *filename)
{
	...
	QuizPlayerCount = CHAR_getPlayerMaxNum();
