...
//		print("ServerType: %d\n", getServerType() );
        ...
        print("登陆服务器名称: %s\n",
              getGameservername());
        /* STONEAGE_REDACT_PASSWORD_LOGS */ print("Account server credential: [redacted]\n");
        ...
        print("日志记载文件: %s\n",  getLogdir());
        print("日志配置文件: %s\n", getLogconffile() );
        /* STONEAGE_REDACT_PASSWORD_LOGS */ print("GM command credential: [redacted]\n");
...
#else
			char buff[50];
			sprintf(buff,"legacy_example_1-%s-%d",getAccountserverpasswd(),123);
