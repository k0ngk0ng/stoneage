...
当前入口为「系统 → 导入资源」，完整游戏 ZIP 的生成、下载与发布见 [游戏资源 ZIP](game-resource-packs.md)。以下保留旧 `.samap` 格式说明；旧包仅在资源版本匹配时可用。

游戏内打开「系统 → 导入资源」，下载常用四村或全部地图包，再选择下载得到的 `.samap` 文件并点击「开始导入」。浏览器会显示文件大小、可用空间及进度。取消、关闭页面或刷新后，重新选择同一个文件即可继续；已完成的文件会校验后复用。

v0.1.72 起游戏内下载入口使用 CDN；v0.1.70 的 Release 附件仍可作为镜像：
...
发版前更新 `client/web/runtimeassets/map-packs.json` 中的资源版本、文件大小、SHA-256 及 CDN 相对路径；使用已发布版本的 `bin/stoneage sync-assets --web-only --map-packs DIR` 上传，GitHub Release 附件可保留作为镜像。下载入口只显示与当前资源版本相符的包。地图包没有改变现有 CDN 图片内容，因此不需要重复上传原来的贴图。
...
地图图号按 DAT 中的逻辑编号解析；清单记录的 `bmp_number` 必须匹配，不能用同号的人物物理帧代替。天空之境（5581）、元神宫（104）及天空岛系列（30689、30691–30695）还依赖原版独立背景图，打包器会将背景加入包中，即使普通地图层全部为空。游戏绘制背景时使用同一图片缓存，并在背景可用后结束进图等待。
