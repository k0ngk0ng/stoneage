...
全部任务有独立奖励表。`rewards.go` 显式关联龙洞、强恩一族、神奇贝壳、马祖、琉璃洞窟双袋、梦幻洞窟、JOT/SOT 的本服脚本，依据 `NPC_RandItemGet` / `NPC_EventAddPet` 计算同一列表内重复 ID 的权重，分别列出道具与宠物抽取池，不把中途退出或阶段奖励混成最终奖。`rand()%N` 的理论比例以近似值展示。其余任务按整理的资料说明奖励，尚未核对本服配置的概率不擅自填成 100%。
