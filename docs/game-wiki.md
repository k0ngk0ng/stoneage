...
全部任务有独立奖励表。`rewards.go` 显式关联90项任务（包括48项村庄委托）的本服发奖脚本，依据 `NPC_RandItemGet` / `NPC_EventAddPet` 计算同一列表内重复 ID 的权重，分别列出道具与宠物抽取池，不把中途退出或阶段奖励混成最终奖。`rand()%N` 的理论比例以近似值展示。6项任务为资格或服务解锁，单独说明其作用；愿藏娃娃任务的最终奖励尚无可核对记录，不填造奖励或概率。原脚本停用的加加村宠物D委托明确显示无法兑换。
