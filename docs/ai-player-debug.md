...
异常导致玩家暂停后，保持暂停，不在后台自动恢复。管理员直接点击“启动”，运行时会确认旧执行已退出、保留旧回合未知结果和额度记录，再以新的观察继续运行；不再需要单独的“恢复确认”。旧执行还未结束时，启动会提示稍后重试，不会重叠启动。

### 在本机执行生产 AI 玩家

在 AI 玩家详情中点击“复制本地运行命令”，在装有 Docker 的本机终端粘贴执行。命令使用 Docker 后台模式运行，不会占用当前终端；需要查看日志时可执行 `docker logs -f stoneage-ai-local-<命令中的名称>`。首次执行会拉取对应已发布的 AI 运行镜像，无需安装 Go、Codex 或配置 SSH。管理端应通过本机可达的 HTTPS 地址访问；也支持直接使用局域网 IP 的 HTTP 地址。命令使用当前管理端地址，所以不要从仅服务器自身可访问的 `127.0.0.1` 地址生成给其他机器使用的命令。

生成命令本身不启动玩家。执行命令后，本地执行器凭该玩家的专用接入凭证连接生产，并按命令中的 `--start` 发起启动。生产保留游戏会话、调度、记忆和审计；Codex 在本机独立容器内执行，配置和会话历史保存在该玩家独立 Docker 数据卷中。命令不含模型 API key，不挂载操作者的 Codex 目录，也不需要 Docker socket。模型地址仍使用管理端保存的设置，因此本机须能访问该模型地址。

详情中显示执行位置和连接状态，并支持手动刷新、15/30/60 秒自动刷新。关掉本地容器后会显示断开；不会改由生产容器偷偷接管。重新运行命令会复用同一数据卷中的连接凭证和历史。异常暂停仍需点击“启动”或重新执行带 `--start` 的命令；网络重连本身不重新启用已暂停的玩家。
