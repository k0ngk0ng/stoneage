...
在 AI 玩家详情中点击“复制本地运行命令”，在装有 Docker 的本机终端粘贴执行。首次执行会拉取对应已发布的 AI 运行镜像，无需安装 Go、Codex 或配置 SSH。管理端应通过本机可达的 HTTPS 地址访问；也支持直接使用局域网 IP 的 HTTP 地址。命令使用当前管理端地址，所以不要从仅服务器自身可访问的 `127.0.0.1` 地址生成给其他机器使用的命令。
