...
`walk` 是挂机用的：战斗之外它按 200 毫秒一个动作的节奏走"两格出去、两格回来，再换一个轴"，
...
共用同一份 `internal/battleauto`。两次走路和一次坐标查询合为一组，默认约每秒 3 步，
实际受网络、地图阻挡和战斗状态影响。

Web 自动战斗／自动练级面板提供「快速战斗」开关（默认关闭，保存在当前浏览器）。
开启时保留地图画面，跳过战斗动画和战斗资源预加载，人物／宠物经验、升级及掉落写入本地
系统聊天记录，不弹 RESULT 框；关闭或人工接管时恢复正常战斗画面。该开关只影响显示，
不改变服务端战斗规则。`sactl` 本身没有图形渲染，始终使用同一套后台战斗逻辑，无需此开关。
...
2. **方向字母有大小写语义，且不是从 `a` 开始的顺序。** 小写 `a`..`h` 是行走，大写是转身；
   字母表由 `cnvServDir()` 的旋转决定（`client/web/runtimeassets/index.html:1669-1676`）。`sactl` 只接受
...
5. **选择窗口的 `select` 字段是 0，选项号在 `data` 里。** 原版客户端点击选择行时发送
   ...
   （`client/web/runtimeassets/index.html:15015-15033`）。行号是**从第一个可选行起算、包含空行**的，
...
`internal/ainavigation` 的地图寻路。它不经过管理端或 Web 会话租约。
